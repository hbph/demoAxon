package net.devworkshop.axon.entities.handlers;
...
import net.devworkshop.axon.events.BaseEvent;

@Component
public class CardQueryEntityManager {

	@Autowired
	private CardRepository accountRepository;
	...
	private CardQueryEntity findExistingOrCreateQueryAccount(String id) {
		return accountRepository.findById(id).isPresent() ? accountRepository.findById(id).get()
				: new CardQueryEntity();
	}

	private CardQueryEntity buildQueryAccount(AccountAggregate accountAggregate) {
		CardQueryEntity accountQueryEntity = findExistingOrCreateQueryAccount(accountAggregate.getId());
	...
	private void persistAccount(CardQueryEntity accountQueryEntity) {
